...
function filterOutBadWallsForMeV2(walls, newWall, players, playersPredicted, me, mePredicted) {
    if (!mePredicted) {
        return false;
    }
    const squares = makeGrid();
    ...
    let totalPlayersMovesAfterWall = 0;
    for (let pi = 0; pi < players.length; pi++) {
    ...
    let totalPlayerMovesBeforeWall = 0;
...
function filterBadWallsV2(_walls, walls, newWall, game, mePredicted, playersPredicted, knownBadWalls, force = false) {
    if (knownBadWalls[newWall.wall.id]) {
        return false;
    }
    const pathAvailable = isPathStillAvailable(walls, newWall.wall, [game.me, ...game.others], knownBadWalls);
    if (!pathAvailable) {
        return false;
    }
    if (force) {
    ...
    return filterOutBadWallsForMeV2(walls, newWall, game.others, playersPredicted, game.me, mePredicted);
}
...
function makeWallsToBlockPlayers(game, _walls, walls, playersPredicted, mePredicted, force = false) {
    const knownBadWalls = {};
    const createdWalls = [];
    for (let plI = 0; plI < playersPredicted.length; plI++) {
        createdWalls.push(...makeWallsToBlockPath(playersPredicted[plI], _walls).map((w, order) => {
            const predicted = {
                wall: w,
                value: 0,
                order,
    ...
    const filteredWalls = createdWalls.filter(w => filterBadWallsV2(_walls, walls, w, game, mePredicted, playersPredicted, knownBadWalls, force));
...
function isAboutToWin(predictedPath) {
    return predictedPath.moves === 2;
}
function tryPlacingBestWallOrMove(game, bestWalls, otherAPredicted, otherBPredicted, mePredicted, buffer) {
    const wallToPlace = bestWalls[0];
    const otherPredicted = game.others[0].id === wallToPlace.playerId
        ? otherAPredicted
        : otherBPredicted;
    const player = game.others[0].id === wallToPlace.playerId
        ? game.others[0]
        : game.others[1];
    if (isWallAdjacent(player, wallToPlace.wall, buffer, otherPredicted.nextDirection)) {
        Actions.placeWall(wallToPlace.wall.x, wallToPlace.wall.y, wallToPlace.wall.d);
    }
    else {
        Actions.move(mePredicted.nextDirection);
    }
}
function shouldISecureMyPath(me) {
    if (me.id === 0) {
        return (me.square.id === '00' ||
            me.square.id === '01' ||
            me.square.id === '08' ||
            me.square.id === '07');
    }
    else if (me.id === 1) {
        return (me.square.id === '80' ||
            me.square.id === '81' ||
            me.square.id === '88' ||
            me.square.id === '87');
    }
                ...
                if (!bestWalls || bestWalls.length === 0) {
                    ...
                    if (isP1AboutToWin) {
                        ...
                        if (wallToPlace) {
                            ...
                            tryPlacingBestWallOrMove(_game, bestWalls, otherAPredicted, otherBPredicted, mePredicted, buffer);
                        }
                    }
                    else if (isP2AboutToWin) {
                        const wallToPlace = bestWalls.find(w => w.playerId === _game.others[1].id);
                        if (wallToPlace) {
                            Actions.placeWallV2(wallToPlace.wall);
                        }
                        else {
