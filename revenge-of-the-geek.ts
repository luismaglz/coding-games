...
class Entity extends Point {
  ...
  constructor(x: number, y: number, type: EntityType, id: number) {
    super(x, y);
    this.id = id;
    this.type = type;
  }
...
class Robot extends Entity {
  item: ItemType;
  currentJob: "MINE" | "RADAR" | "TRAP" = "MINE";
  constructor(
    x: number,
    y: number,
    type: EntityType,
  ...
  move(x: number, y: number, message = "") {
...
class Cell extends Point {
  hole: number = 0;
  ore: string | number = "?";
  previousOre: number | string = this.ore;
  previousHole: number = this.hole;
  isBad: boolean = false;
  constructor(ore: number | string, hole: number, x: number, y: number) {
    super(x, y);
  ...
  setIsBad(game: Game) {
    ...
    const cell = game.radarLocations.find(
      c => c.x === this.x && c.y === this.y
    );
    if (cell) {
      this.isBad = true;
    }
  ...
  update(ore: string | number, hole: number) {
...
class Game {
  ...
  traps: Entity[] = [];
  myRobots: Robot[] = [];
  enemyRobots: EnemyRobot[] = [];
  start: number = 5;
  radarLocations: Point[] = [];
  originalRadars: Point[] = [];
  constructor() {
    this.grid = new Grid();
    this.grid.init();
    this.myScore = 0;
...
while (true) {
  ...
  if (game.radars.length !== 19 - game.radarLocations.length) {
    ...
    game.radarLocations = game.originalRadars.filter(
      r => game.radars.findIndex(gr => gr.x === r.x && gr.y === r.y) === -1
    );
  }
  ...
  let availableUnknownCells: Cell[] = [];
  if (nextRadar) {
  ...
  game.myRobots.forEach(robot => {
    if (robot.isDead()) {
      ...
      let location: Point;
      if (
        game.grid.getCell(game.radarLocations[0].x, game.radarLocations[0].y)!
          .isBad === true
      ) {
        location = new Point(
          game.radarLocations[0].x - 1,
          game.radarLocations[0].y
        );
      } else {
      ...
      robot.dig(
        availableUnknownCells[0].x,
        availableUnknownCells[0].y,
        "random"
      );
